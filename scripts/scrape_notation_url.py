...
def download_image_to_project(root: Path, song_id: str, image_url: str, *, timeout: int = 30) -> str:
    """
    Download image from URL into images/ and return relative path for JSON.

    The same returned path can be used for both thumbnail and background.
    """
...
def notesandsargam_html_to_lines(raw_html: str) -> List[str]:
    """
    Extract text lines from notesandsargam.com pages.

    These pages are Elementor-heavy and generic article extraction often lands in
    comments instead of the song notation block. This parser anchors around the
    explicit song-notation intro and trims before Hindi/legend/share sections.
    """
    body = raw_html
    ...
    cut_patterns = [
        r"Sargam\s+notes\s+in\s+Hindi",
        r"How\s+to\s+read\s+SARGAM",
        r"share\s+this\s+on",
        r"Leave\s+a\s+Reply",
    ]
...
_SKIP_PATTERNS = [
    ...
    re.compile(r"^\s*Reply\s*$", re.IGNORECASE),
    re.compile(r"^\s*Leave\s+a\s+Reply", re.IGNORECASE),
    re.compile(r"^\s*share\s+this\s+on", re.IGNORECASE),
...
def _is_sargam_line(line: str) -> bool:
    ...
    # notesandsargam style, e.g. "P PM | DP...M | M~GG | R R | G"
    if "|" in t and re.search(r"[SRGmMPDNrgdn]", t):
        alpha_chars = re.findall(r"[A-Za-z]", t)
        if alpha_chars:
            note_alpha = sum(1 for c in alpha_chars if c in _SARGAM_LETTERS)
            # Reject natural-language lyrics such as "Kabhi | kabhi | mere..."
            if note_alpha / len(alpha_chars) < 0.75:
                return False
        tokens = re.findall(r"[.,]?[SRGmMPDNrgdn](?:[\'#\^]|\([^)]*\)|\{[^}]*\})?", t)
        if len(tokens) >= 3:
            return True
...
def _extract_metadata(title: str, lines: List[str]) -> Tuple[str, List[str]]:
    ...
    for line in lines[:20]:
        ...
        if m:
            k = m.group(1).strip()
            v = m.group(2).strip()
            if k.lower().startswith("flute used"):
                k = "Flute"
            if k.lower() == "music director":
                k = "Music Director"
            info.append(f"{k}: {v}")
            continue
    ...
    try:
        ...
        for sec in sections:
            for ln in sec.get("lines", []):
                indian = ln.get("indian", "")
                if indian:
                    ln["western"] = indian_to_western(indian, flat)
    except Exception:
        pass  # western notes will be filled by add_western.py or Hugo build
...
def _normalize_notation_symbolic(raw: str) -> str:
    """Normalize spacing while preserving symbolic sargam markers."""
    s = raw.strip()
    if not s:
        return s
    ...
    # Normalize separator spacing but preserve symbolic markers.
    s = re.sub(r"\s*\|\s*", " | ", s)
    s = re.sub(r"\s+", " ", s).strip()
    return s

def _pair_lyrics_and_notation(lines: List[str], *, keep_symbolic: bool = False) -> List[Dict[str, Any]]:
    """
    ...
    while i < len(lines):
        ...
        if _is_lyrics_line(line):
            ...
            if i + 1 < len(lines) and _is_sargam_line(lines[i + 1]):
                indian = (
                    _normalize_notation_symbolic(lines[i + 1])
                    if keep_symbolic
                    else _normalize_notation(lines[i + 1])
                )
                i += 2
            else:
                i += 1
        ...
        if _is_sargam_line(line):
            indian = _normalize_notation_symbolic(line) if keep_symbolic else _normalize_notation(line)
            current_section["lines"].append({
                "lyrics": "",
                "indian": indian,
            })
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="",
        help=(
            "Image URL for thumbnail/background. If omitted (and not --dry-run), "
            "you will be prompted before saving each song."
        ),
    )
    args = ap.parse_args()
    ...
    for url in args.urls:
        try:
            ...
            if args.dry_run:
                ...
                if not image_url:
                    image_url = input(
                        "Enter thumbnail/image URL (optional, press Enter to skip): "
                    ).strip()

                if image_url:
                    try:
                        rel_image_path = download_image_to_project(
                            ROOT,
                            str(song["id"]),
                            image_url,
                        )
                        song["thumbnail"] = rel_image_path
                        song["background"] = rel_image_path
                        print(f"🖼️ Saved image: {rel_image_path}")
                    except Exception as img_err:
                        print(
                            f"⚠️ Could not download image ({img_err}). "
                            "Continuing with default image paths.",
                            file=sys.stderr,
                        )
